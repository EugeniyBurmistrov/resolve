...
const createHashRunTime = (prefix, contentHolder) => `(() => {
   const hmac = __non_webpack_require__('crypto').createHmac('sha512', ${JSON.stringify(
     prefix
   )})
  hmac.update(${contentHolder})
  return hmac.digest('hex')
})()`
...
const importInstanceResource = ({
  ...
  if (!checkRuntimeEnv(resourceValue)) {
    ...
    constants.push(
      `const ${resourceName} = interopRequireDefault(`,
      `  __non_webpack_require__(${injectRuntimeEnv(resourceValue)})`,
      `).default`
    )

    if (calculateHash != null) {
      constants.push(
        `const ${resourceName}_file = __non_webpack_require__('fs')
          .readFileSync(${injectRuntimeEnv(resourceValue)}).toString()`
      )
...
const importConstructorResourceModule = ({
  ...
  if (!checkRuntimeEnv(module)) {
    ...
    constants.push(
      `const ${resourceName}_constructor = interopRequireDefault(`,
      `  __non_webpack_require__(${injectRuntimeEnv(module)})`,
      `).default`
    )

    if (calculateHash != null) {
      constants.push(
        `const ${resourceName}_constructor_file = __non_webpack_require__('fs')
          .readFileSync(${injectRuntimeEnv(module)}).toString()`
      )

      constants.push(
        `const ${resourceName}_constructor_hash = ${createHashRunTime(
          calculateHash,
          `${resourceName}_constructor_file`
        )}`
      )
    }
  }
...
const importConstructorResourceImports = ({
  ...
  for (const importKey of Object.keys(resourceImports)) {
    ...
    if (!checkRuntimeEnv(importValue)) {
      ...
      constants.push(
        `const ${inlineImportKey} = interopRequireDefault(`,
        `  __non_webpack_require__(${injectRuntimeEnv(importValue)})`,
        `).default`
      )

      if (calculateHash != null) {
        constants.push(
          `const ${inlineImportKey}_file = __non_webpack_require__('fs')
            .readFileSync(${injectRuntimeEnv(importValue)}).toString()`
        )

        constants.push(
          `const ${inlineImportKey}_hash = ${createHashRunTime(
            calculateHash,
            `${inlineImportKey}_file`
          )}`
        )
      }
    }
  ...
  if (calculateHash != null) {
    constants.push(
      `const ${resourceName}_imports_hash_aggregate = JSON.stringify(${inlinedImports
      ...
      `const ${resourceName}_imports_hash = ${createHashRunTime(
        calculateHash,
        `${resourceName}_imports_hash_aggregate`
      )}`
    )
  }
}
...
const importConstructorResourceOptions = ({
  ...
  if (calculateHash != null) {
    constants.push(
      `const ${resourceName}_options_hash_aggregate = JSON.stringify(${resourceName}_options)`
    )

    constants.push(
      `const ${resourceName}_options_hash = ${createHashRunTime(
        calculateHash,
        `${resourceName}_options_hash_aggregate`
      )}`
    )
  }
}
...
const importConstructorResource = ({
  ...
  if (calculateHash != null) {
    constants.push(
      `const ${resourceName}_hash_aggregate = JSON.stringify([
